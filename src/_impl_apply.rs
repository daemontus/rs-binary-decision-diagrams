...
#[inline]
fn apply<T: Fn(Option<bool>, Option<bool>) -> Option<bool>>(left: &Bdd, right: &Bdd, terminal_lookup: T) -> Bdd {
    let mut result = Bdd::new_true();
    result.ensure_variables(max(left.variable_count(), right.variable_count()));
    ...
    while let Some(on_stack) = stack.last() {
        if task_cache.contains_key(on_stack) {
            ...
            let new_low = terminal_lookup(l_low.as_bool(), r_low.as_bool())
                .map(Pointer::from_bool)
                .or_else(|| task_cache.get(&task_low).cloned());
            let new_high = terminal_lookup(l_high.as_bool(), r_high.as_bool())
                .map(Pointer::from_bool)
                .or_else(|| task_cache.get(&task_high).cloned());
