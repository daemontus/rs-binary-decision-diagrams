...
impl Bdd {
    ...
    pub(crate) fn pointers_of(&self, pointer: Pointer) -> PointerPair {
        unsafe {
            ...
            let low_ref: *const (u32, u16, Variable, PointerPair) =
                self.nodes.get_unchecked(low.0 as usize);
            std::arch::x86_64::_mm_prefetch::<3>(low_ref as (*const i8));
            let high_ref: *const (u32, u16, Variable, PointerPair) =
                self.nodes.get_unchecked(high.0 as usize);
...
impl TryFrom<&str> for Bdd {
    ...
    fn try_from(data: &str) -> Result<Self, Self::Error> {
        ...
        for node_string in data.split('|').filter(|s| !s.is_empty()) {
            ...
            let left_pointer = if let Ok(x) = left_pointer.unwrap().parse::<u32>() {
                x
            } else {
                return Err(format!(
            ...
            let right_pointer = if let Ok(x) = right_pointer.unwrap().parse::<u32>() {
                x
            } else {
                return Err(format!(
                    "Invalid pointer numeral `{}`.",
            ...
            nodes.push((
                ...
                0,
                Variable(variable),
                Pointer(left_pointer) | Pointer(right_pointer),
            ));
        }
        Ok(Bdd { nodes })
    }
