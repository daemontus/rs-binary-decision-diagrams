...
/// A task/result stack used when performing the "coupled DFS" routine.
...
/// into a terminal.
pub fn apply<TABLE>(left_bdd: &Bdd, right_bdd: &Bdd, lookup: TABLE) -> Bdd where TABLE: Fn(NodeId, NodeId) -> NodeId {
...
impl Bdd {
    ...
    #[no_mangle]
    pub fn and_not_u48(&self, other: &Bdd) -> Bdd {
        apply_u48!(self, other,
            |l: NodeId, r: NodeId| l.is_zero() || r.is_one(),
            |l: NodeId, r: NodeId| l.is_one() && r.is_zero()
        )
    }
